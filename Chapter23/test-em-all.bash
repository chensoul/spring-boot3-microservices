...
function testCircuitBreaker() {
    ...
    if [[ $USE_K8S == "false" ]]
    then
        # Use the authorization to run curl, since the product-composite is native compiled
        EXEC="docker compose exec -T authorization"
    else
        # Use the authorization to run curl, since the product-composite is native compiled
        EXEC="kubectl -n $NAMESPACE exec deploy/authorization -c authorization -- "
    fi

    # First, use the health - endpoint to verify that the circuit breaker is closed
    assertEqual "CLOSED" "$($EXEC curl -s http://product-composite:${MGM_PORT}/actuator/health | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "OPEN" "$($EXEC curl -s http://product-composite:${MGM_PORT}/actuator/health | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "HALF_OPEN" "$($EXEC curl -s http://product-composite:${MGM_PORT}/actuator/health | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "CLOSED_TO_OPEN"      "$($EXEC curl -s http://product-composite:${MGM_PORT}/actuator/circuitbreakerevents/product/STATE_TRANSITION | jq -r .circuitBreakerEvents[-3].stateTransition)"
    assertEqual "OPEN_TO_HALF_OPEN"   "$($EXEC curl -s http://product-composite:${MGM_PORT}/actuator/circuitbreakerevents/product/STATE_TRANSITION | jq -r .circuitBreakerEvents[-2].stateTransition)"
    assertEqual "HALF_OPEN_TO_CLOSED" "$($EXEC curl -s http://product-composite:${MGM_PORT}/actuator/circuitbreakerevents/product/STATE_TRANSITION | jq -r .circuitBreakerEvents[-1].stateTransition)"
}
